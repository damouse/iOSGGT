...
- (void)drawBackgroundWithRect:(CGRect)rect
{
    ...
    CGContextSaveGState(ctx);
    {
        ...
        UIBezierPath* shadow = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0.5, 0, rect.size.width - 1, rect.size.height - 1) cornerRadius:self.radius];
        [shadow stroke];
...
-(void)activateAnimation:(NSTimer*)timer {    
    if(self.progress >= self.maxValue)
            [self stopAnimation:self];
